@echo off
REM ════════════════════════════════════════
REM   Content Image Automation - Einrichtung (Windows)
REM ════════════════════════════════════════
REM Doppelklick auf diese Datei richtet alles ein.

cd /d "%~dp0"
echo ========================================
echo   Content Image Automation - Einrichtung (Windows)
echo ========================================

REM - HIER ANPASSEN, falls dein Repo anders heisst: -
set "REPO_URL=https://github.com/MoritzAtCaretable/Content-Image-Automation.git"
...
REM 2. git pruefen, ggf. per winget installieren
where git >nul 2>&1
if errorlevel 1 (
...
REM 2b. Falls KEIN Git-Checkout (ZIP-Download): nachtraeglich Git-Verbindung
REM      herstellen, damit der Update-Button funktioniert. Es wird NICHTS geloescht -
REM      nur der .git-Ordner wird aufgepfropft; .env und secrets\ bleiben.
...
echo   1. .env oeffnen und GEMINI_API_KEY + GOOGLE_SHEET_ID eintragen
...
echo   4. "Content Image Automation.bat" per Doppelklick starten
...
set "TMP_CLONE=%TEMP%\cia_graft_%RANDOM%"
